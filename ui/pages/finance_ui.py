"""
WaldosLair — Finance modul UI (Wayne Enterprises themed).

Sadrži sve Streamlit UI komponente za Finance modul:
  - Data editor s real-time sinkronizacijom s bazom
  - Tab rendereri po računima
  - Modal za unos transakcija
  - KPI kartice i financijski reporti
  - Javna metrika za Dashboard karticu

Sav pristup bazi ide isključivo kroz database.py.
Sva formatiranja valute su u utils.py.
"""
...
from styles import WAYNE_CSS
from utils import format_eur, parse_eur, safe_int, sanitize_text
...
def kpi(label: str, value: str, tone: str = "") -> str:
    """
    Generiraj HTML za KPI karticu.

    Args:
        label: Naziv metrike
        value: Formatirana vrijednost (npr. '1.234,56 €')
        tone: CSS klasa — 'pos' (zelena), 'neg' (crvena), '' (bijela)
    """
    css_class = f"kpi-value {tone}".strip()
    # Sanitizacija — label i value dolaze iz interne logike, ali budimo sigurni
...
# ---------------------------------------------------------------------------
# Data editor — konfiguracija kolumni
# ---------------------------------------------------------------------------

def _editor_column_config(account_locked: str | None = None) -> dict:
    """Konfiguracija prikaza kolumni za st.data_editor."""
    return {
        ...
        "category": st.column_config.SelectboxColumn(
            "Kategorija", options=ALL_CATEGORIES, required=True
        ),
...
def _apply_editor_changes(
    ...
    account_locked: str | None,
) -> bool:
    """
    Primijeni promjene iz st.data_editor session state-a na bazu.

    Returns:
        True ako je bila ikakva promjena (triggera st.rerun())
    """
    ...
    for idx in state.get("deleted_rows", []) or []:
        try:
            ...
            logger.warning("Greška pri brisanju retka %d: %s", idx, e)
            continue

    # Editiranje postojećih redova
    for idx, edits in (state.get("edited_rows") or {}).items():
        try:
            ...
            logger.warning("Greška pri ažuriranju retka %s: %s", idx, e)
            continue
    ...
    for new_row in state.get("added_rows") or []:
        ...
        try:
            ...
            logger.error("Greška pri unosu novog retka: %s", e)
            st.error(f"Greška pri spremanju: {e}")

    # Resetiraj stanje editora kako ne bi ponovo aplicirao iste promjene
    state["deleted_rows"] = []
    state["edited_rows"] = {}
    state["added_rows"] = []
    return changed
...
def render_editor(
    ...
    """
    Prikaži editabilnu tablicu transakcija koja se sinkronizira s bazom.

    Args:
        df: DataFrame s transakcijama
        editor_key: Jedinstveni Streamlit ključ za editor
        account_locked: Ako je postavljeno, račun je zaključan na tu vrijednost
        hide_account: Sakrij stupac računa (za tab-specific prikaze)
    """
    show_df = df.copy()
...
# Fix 4: @st.dialog ima bug s Python 3.12+ (TypedDict.__new__ error).
# Koristimo session_state modal pattern koji radi na svim verzijama Streamlita.

def _render_new_transaction_form() -> None:
    """Form za novu transakciju — renderira se inline kad je modal otvoren."""
    st.markdown("### ➕ Nova Transakcija")
    st.markdown("<div class='wayne-divider'></div>", unsafe_allow_html=True)

    tx_type = st.segmented_control(
        "Tip", TRANSACTION_TYPES, default="Trošak", key="dlg_type"
    )
    if not tx_type:
        tx_type = "Trošak"
    ...
    with c3:
        tx_date = st.date_input(
            "Datum", value=date.today(), key="dlg_date", format="DD.MM.YYYY"
        )
    with c4:
        amount_text = st.text_input(
            "Iznos (€)",
            value="0,00",
            placeholder="npr. 1.234,56",
            help="Format: 1.234,56 ili 100",
            key="dlg_amount",
        )
...
def _account_balance(df: pd.DataFrame) -> float:
    """Izračunaj neto stanje računa iz DataFrame-a."""
    if df.empty:
        return 0.0
...
def tab_checking() -> None:
    """Tekući račun — početno stanje se podešava u tabu Postavke."""
    ...
    st.caption(
        f"Početno stanje: {format_eur(starting)} · "
        f"Neto transakcija: {format_eur(_account_balance(df))}"
    )
    st.markdown("<div class='wayne-divider'></div>", unsafe_allow_html=True)
...
def tab_mastercard() -> None:
    """MasterCard Kreditni — s filtrom datumskog raspona."""
    today = date.today()
    c1, c2 = st.columns(2)
    with c1:
        start = st.date_input(
            "Od datuma", value=today.replace(day=1), key="mc_start", format="DD.MM.YYYY"
        )
    with c2:
    ...
    expenses = (
        float(filtered.loc[filtered["type"] == "Trošak", "amount"].sum())
    ...
    with k2:
        st.markdown(
            kpi("Neto u rasponu", format_eur(balance), "pos" if balance >= 0 else "neg"),
            unsafe_allow_html=True,
        )

    st.markdown("<div class='wayne-divider'></div>", unsafe_allow_html=True)
    render_editor(
        filtered, editor_key="ed_mastercard", account_locked="MasterCard Kreditni", hide_account=True
    )


def _simple_account_tab(account: str, editor_key: str) -> None:
    """Generički tab za jednostavne račune (Keks, Revolut)."""
    ...
    with k1:
        st.markdown(
            kpi("Stanje", format_eur(balance), "pos" if balance >= 0 else "neg"),
            unsafe_allow_html=True,
        )
    with k2:
...
def tab_njivice() -> None:
    """Njivice — troškovi podijeljeni na 3 vlasnika s grafom."""
    ...
    if not df.empty:
        ...
        monthly = chart_df.groupby("month")["amount"].sum().reset_index()
        monthly = monthly.sort_values("month")
        st.markdown("**Mjesečni troškovi**")
        st.bar_chart(monthly, x="month", y="amount", color="#4a90d9", height=260)
    else:
        st.info("Još nema podataka za prikaz mjesečnog grafa.")
...
def tab_loan() -> None:
    ...
    monthly = (
        parse_eur(get_setting("loan_monthly", str(LOAN_MONTHLY_DEFAULT)))
        or LOAN_MONTHLY_DEFAULT
    )
    total = safe_int(
        get_setting("loan_total", str(LOAN_TOTAL_DEFAULT)), LOAN_TOTAL_DEFAULT, min_value=1
    )
    paid = max(
        0,
        min(
            total,
            safe_int(
                get_setting("loan_paid", str(LOAN_PAID_DEFAULT)),
                LOAN_PAID_DEFAULT,
                min_value=0,
            ),
        ),
    ...
    with st.expander("⚙️ Postavke kredita", expanded=False):
        c1, c2, c3 = st.columns(3)
        with c1:
            m_text = st.text_input(
                "Mjesečni anuitet (€)",
                value=format_eur(monthly).replace(" €", ""),
                key="loan_m_input",
            )
        with c2:
            t_text = st.text_input("Ukupan broj rata", value=str(total), key="loan_t_input")
        with c3:
            p_text = st.text_input("Broj uplaćenih rata", value=str(paid), key="loan_p_input")

        if st.button("💾 Spremi postavke kredita", key="loan_save", type="primary"):
    ...
    with k2:
        st.markdown(
            kpi("Preostali iznos", format_eur(remaining_amount), "neg"), unsafe_allow_html=True
        )
    with k3:
        st.markdown(
            kpi("Preostalih rata", str(remaining_installments)), unsafe_allow_html=True
        )
    ...
    st.markdown(
        ...
        f"Plaćeno {paid}/{total} rata &nbsp;·&nbsp; {pct * 100:.1f}%"
        f"</div>",
        unsafe_allow_html=True,
    )
...
def tab_reports() -> None:
    """Reporti — tjedni, mjesečni, master ledger i CSV export."""
    df = fetch_all()
    ...
    with sub_tabs[0]:
        if df.empty:
            ...
            st.bar_chart(
                grouped.set_index("week")[["Prihodi", "Troškovi"]], height=280
            )
    ...
    with sub_tabs[1]:
        if df.empty:
            ...
            with k3:
                st.markdown(
                    kpi("Neto", format_eur(net), "pos" if net >= 0 else "neg"),
                    unsafe_allow_html=True,
                )
    ...
    with sub_tabs[3]:
        st.markdown("Preuzmi cijeli skup podataka kao CSV.")
        if df.empty:
...
def tab_settings() -> None:
    ...
    with c1:
        sb_text = st.text_input(
            "Početno stanje (€)",
            value=format_eur(starting).replace(" €", ""),
            placeholder="npr. 1.234,56",
            key="settings_sb_tekuci",
        )
    with c2:
    ...
    paid = max(0, min(total, safe_int(
        get_setting("loan_paid", str(LOAN_PAID_DEFAULT)), LOAN_PAID_DEFAULT, min_value=0
    )))
...
def get_finance_dashboard_metrics() -> dict:
    """
    Vrati real-time financijske KPI-je za karticu na glavnom dashboardu.

    Returns dict s ključevima:
        - trenutno_stanje: Stanje Tekućeg računa (sa početnim stanjem)
        - troskovi_mjesec: Ukupni troškovi svih računa u tekućem mjesecu
        - predvideno_stanje: trenutno_stanje minus nenaplaćeni MasterCard dug
    """
    init_db()
    starting = parse_eur(get_setting("starting_balance_tekuci", "0"))
    df = fetch_all()
    ...
    if df.empty:
        return {
            "trenutno_stanje": float(starting),
            "troskovi_mjesec": 0.0,
            "predvideno_stanje": float(starting),
        }
    ...
    mc_mask = (
        month_mask
        & (df["account"] == "MasterCard Kreditni")
        & (df["type"] == "Trošak")
    )
    mc_pending = float(df.loc[mc_mask, "amount"].sum())

    return {
        "trenutno_stanje": trenutno,
        "troskovi_mjesec": troskovi,
        "predvideno_stanje": trenutno - mc_pending,
    }


# ---------------------------------------------------------------------------
# Glavni entry point za Finance modul
# ---------------------------------------------------------------------------

def render() -> None:
    """Renderira kompletan Finance modul."""
    init_db()
    ...
    # Prikaži inline formu ako je modal otvoren
